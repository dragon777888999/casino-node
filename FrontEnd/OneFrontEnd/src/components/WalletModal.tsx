...
const WalletModal: React.FC<WalletModalProps> = ({ isOpen, onRequestClose }) => {
    ...
    const onDeposit = async () => {
        ...
        const nextAta = getAssociatedTokenAddressSync(new PublicKey(tokenAddress), new PublicKey(depositAddress), true)
        ...
        transaction.add(createAssociatedTokenAccountIdempotentInstruction(wallet.publicKey, nextAta, new PublicKey(depositAddress), new PublicKey(tokenAddress)))
        ...
        const transactionSignature = await wallet.sendTransaction(transaction, connection, { skipPreflight: true, preflightCommitment: "finalized" });
    ...
    return (
        <Modal id="modal" className="modal" isOpen={isOpen}
                ...
                <div className="modal-content">
                    ...
                    <div className="withdraw">
                        <section>
                            ...
                            <p style={{ fontSize: `26px`, margin: 0 }}>{userInfo.balances[userInfo.selectedCoinType]} {userInfo.selectedCoinType}</p>
                        </section>
                        <section>
                            <p style={{ fontSize: `12px` }}>withdraw Amount</p>
                            <input type="number" id="withdrawAmount" name="withdrawAmount" placeholder="" value={withdrawAmount} onChange={(e) => { setWithdrawAmount(Number.parseFloat(e.target.value.toString())) }} />
                        </section>
                        <section>
                    ...
                    <div className="deposit">
                        <section>
                            ...
                            <input type="text" id="depositAddress" name="depositAddress" placeholder="" value={depositAddress} readOnly />
                        </section>
                        {/*}                        <section>
                            <p style={{ fontSize: `12px` }}>Deposit Amount</p>
                            <input type="number" id="depositAmount" name="depositAmount" placeholder="" />
                        </section>
                        <div>

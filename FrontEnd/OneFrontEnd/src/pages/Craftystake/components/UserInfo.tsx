...
import { backendUrl, entryUrl } from "../../../anchor/setup";
import {
  useAnchorWallet,
  useConnection,
  useWallet,
  ...
  const updateUserInfo = async () => {
    try {
      const response = await fetch(
        `${backendUrl}/Account/UserInfo?domain=${entryUrl}`,
        {
          method: "GET",
          headers: {
        ...
        try {
          ...
          if (siteInfo.agentCode == "") {
            const response1 = await fetch(
              `${backendUrl}/Account/SiteInfo?domain=${entryUrl}`
            );
  ...
  return (
    <div className="subHeader">
      <div className="position: relative">
        <div
          className="balance"
          style={{ backgroundColor: "rgb(33, 199, 214)" }}
        >
          <span> {nickName} </span>
          <SelectCoinTypeMenu
          ...
          >
            <img
              src="/csTheme/images/arrowDown.png"
              alt=""
        ...
        >
          ...
          >
            <p> MANAGE BALANCE </p>
            <img src="/csTheme/images/arrowRight.png" />
          </button>

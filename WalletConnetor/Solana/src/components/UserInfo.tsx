...
const UserInfo = () => {
        ...
        const handleWalletConnect = async () => {
            if (wallet.connected) {
                try {
                    ...
                    const response = await fetch(`${backendUrl}/Account/ConnectWallet?agentCode=${siteInfo.agentCode}&userCode=${wallet.publicKey}`);
    ...
    const [selectedKey, setSelectedKey] = useState<string>(userInfo.selectedCoinType);

    const handleSelect = async (key: string) => {
        userInfo.selectedCoinType = key;
        const response = await fetch(`${backendUrl}/Account/SwitchCoinType?coinType=${key}`, {
            method: 'GET',
            headers: {
                'X-Access-Token': accessToken
            },
        ...
        <div className="subHeader">
            <div className="position: relative">
                <div className="balance">
                    <span> {nickName} </span>
                    <SelectCoinTypeMenu items={userInfo.balances} selectedKey={selectedKey} onSelect={handleSelect} />
                    ...
                    <button className="arrowDown" id="arrowBtn" onClick={showMenu}>
                        <img src="/enTheme2/images/arrowDown.png" alt="" style={{ width: '13px', marginLeft: '7px' }} />
                    </button>
                </div>
                <div className="menu" id="menu" style={{ display: isMenuVisible ? 'block' : 'none' }} onMouseLeave={hideMenu} >

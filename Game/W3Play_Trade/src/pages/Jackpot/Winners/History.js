...
const JackpotHistory = ({ isVisibe, toggleJackpotHistory }) => {

    const { getHistoryList, historyList } = useJackpotState(),
        ...
        <div className={`jackpot-history${isVisibe ? '' : ' hidden'}`}>
            {
                ...
                <HelmetManager
                    ...
                    description="Jackpot History: Review Past Winners, Performance Trends, and Giveaway Transactions on the blockchain."
                    keywords="jackpot monthly history, Jackpot winners, crypto winners,crypto payouts UpvsDown"
                    canonical="/jackpot_history"
                />
               : null
            }
            ...
            <div className="title">
                <span>{APP.term('weekly_jackpot_history_title')}</span>
            </div>
            ...
            <div className="table">
                ...
                <div className="table-header">
                    ...
                    <div className="rank">{APP.term('weekly_jackpot_history_place')}</div>
                    <div className="prize"><span>{APP.term('weekly_jackpot_history_prize')}</span></div>
                </div>


                <div className="table-body">
                    {historyList?.sort((a, b) => new Date(b.date) - new Date(a.date))?.map((itm, i) => (

...
import graphLengthFactor from '../utils/graphLengthFactor';
...
function AssetHistoryGraph({ marketHeight, padding }) {
	...
	const { downtime, duration: trade_duration } = active_table;

	//const phase = useAppState('phase');

	const location = useLocation();

	const [length_factor, set_length_factor] = useState(graphLengthFactor(active_table.pool_id, phase));
	const currentGraphFactor = graphLengthFactor(active_table.pool_id);
	...
	useEffect(() => {
		...
		// let factors = APP.state.get('graph_length_factor');
		let factors = graphLengthFactor(active_table.pool_id);
			...
			<svg width={width} height={height}>
				...
				<Group>
					...
					<LinePath
						...
						strokeWidth={vw(isMobilePortrait ? .65 : .2)}
						data={visibleData}
...
export default React.memo(AssetHistoryGraph);